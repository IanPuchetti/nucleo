...
.controller('campanias', function($scope, sharedVars, $http) {
		var _=$scope;

		_.data=sharedVars.datos;
		_.data.campanias= function (){
			$scope.campanias=[];
			$http.post('php/campanias.php', {id: id_usuario}).then(function(res){
			$scope.campanias=res.data;
			});
		};
		_.data.agendas= function (){
			$scope.agendas=[];
			$http.post('php/agendas.php', {id: id_usuario, fecha: hoy}).then(function(res){
		...
		_.data.seleccionar = function (id,id_campania){	
										_.data.id_campania=id_campania;
		...
		_.data.campanias();_.data.agendas();
})
.controller('gestion', function($scope, sharedVars, socket, $timeout, $http){
		...
		_.registrar = function (){
						 _.gestion.opcion=_.data.opcion;
						...
						_.gestion.hoy=hoy;
						_.gestion.id_caso=_.data.deudor.documento;
						_.gestion.id_campania=_.data.id_campania;
