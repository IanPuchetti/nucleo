...
app.controller('myCtrl', function($http, $scope) {
			...
			_.cambio_tipo= function (n) {
													if(_.productos[n].tipo == 'Cuenta-Corriente' || _.productos[n].tipo == 'Tarjeta'){
			...
			_.recargar = function () {
													...
													for (var i in productos){
														...
														p.dias_atraso=calcularDiasDate2(_.fecha_actualizacion, p.fecha_mora);
														p.tasa = _.tasa;
														if(p.tipo=='Prestamo'){
															p.interes=(_.dias_para_cancelar + 40 ) * p.tasa / 365 * parseFloat(p.deuda) * 1.21;
														}else{
															p.interes=(p.dias_atraso* p.tasa / 365) * parseFloat(p.deuda);
														}
														p.suma=parseFloat(p.deuda)+parseFloat(p.interes);
			...
			$http.post('php/datos.php', datos)
					   .then(function (res){
										   ...
										   _.email=res.email;
  										   ...
  										   _.calcular_cuotas; 
  										   _.quitar();
  										   _.fecha_propuesta=new Date();

  				_.descargar = function (){
  				$http.post('http://'+url+':3001/liquidacion-credicoop',{	
  															banco:_.banco,
