...
<table class="table">
	...
	<tr ng-show="datos.cuota_cero==0"><td>Pagos </td><td> {{datos.pagos}}</td></tr>
	<tr><td ng-show="datos.cuota_cero==0">Anticipo: ${{datos.anticipo}}</td><td ng-show="datos.cuota_cero==1">Anticipo: ${{datos.anticipo_cuota_cero}}</td></tr>
	<tr ng-show="datos.cuota_cero==1"><td>Cuotas banco: {{datos.pagos}}</td><td>Monto por cuota: ${{datos.monto_cuota}}</td></tr>
	<tr ng-show="datos.cuota_cero==1"><td>Cuotas honorarios: {{datos.cuotas_honorarios}}</td><td>Monto por cuota: ${{datos.monto_honorarios | number:2}}</td></tr>
	<tr ng-show="datos.cuota_cero==0"><td>Cuota promedio </td><td> ${{datos.cuota_promedio | number: 0}}</td></tr>
	<tr><td>Total promedio </td><td ng-show="datos.cuota_cero==0"> ${{datos.total_promedio | number: 0}}</td><td ng-show="datos.cuota_cero==1"> ${{datos.total_total}}</td></tr>
</table>
<table class="table">
	<tr class="table-header">
		<td colspan=3 style="text-align:center;">PROPUESTA</td>
	</tr>
	<tr><td>Fecha de propuesta: {{datos.fecha_propuesta | date: "dd/MM/yyyy"}}</td><td colspan=2>Fecha de pago: {{datos.fecha_pago | date: "dd/MM/yyyy"}}</td></tr>
	<tr><td ng-show="datos.cuota_cero==0">Anticipo: ${{datos.anticipo}}</td><td ng-show="datos.cuota_cero==1">Anticipo: ${{datos.anticipo_cuota_cero}}</td><td colspan=2>Monto total: ${{datos.total_total}}</td></tr>
