...
app.controller('myCtrl', function($http, $scope) {
			...
			_.cambio_producto= function () {
													if(_.producto == 'Cuenta-Corriente'){
														_.tasa = _.tasa_cuenta_corriente;
													}else{
														if(_.producto == 'Tarjeta'){
															_.tasa = _.tasa_tarjeta;
														}else{
															if(_.producto == 'Prestamo'){
																_.tasa = _.tasa_prestamo;
															}
														}
													}	
											};

			_.cambio_tipo= function (n) {
													if(_.productos[n].tipo == 'Cuenta-Corriente'){
														_.productos[n].tasa = _.tasa_cuenta_corriente;
													}else{
														if(_.productos[n].tipo == 'Tarjeta'){
															_.productos[n].tasa = _.tasa_tarjeta;
														}else{
															if(_.productos[n].tipo == 'Prestamo'){
																_.productos[n].tasa = _.tasa_prestamo;
															}
														}
													}	
											};
			...
			_.quitar = function () {    if (_.estado == 2){
															_.producto_quita.aviso="ANTICIPO DEL 20%. HASTA 6 CUOTAS SIN QUITA";
			}else{
										if(_.total < 15000){
													var p;
													_.producto_quita = {};
													var a = _.producto_quita;
													a.sub_total=0;
													for (var i in _.productos){
														p=_.productos[i];
														a.sub_total=parseFloat(a.sub_total)+parseFloat(p.deuda);
													}
													a.honorarios=parseFloat(a.sub_total)*(parseFloat(_.porcentaje_honorarios)/100);
													a.iva_sobre_honorarios=parseFloat(a.honorarios)*0.21;
													a.total=parseFloat(a.sub_total)+parseFloat(a.honorarios)+parseFloat(a.iva_sobre_honorarios);
													a.aviso="CAPITAL + HONORARIOS + IVA S/HONORARIOS = TRES CUOTAS SIN INTERES";
													a.cuotas=a.total/3;
										}else{
											if(_.sub_total >= 1500 && _.sub_total < 60000){
												var p;
													_.producto_quita = {};
													var a = _.producto_quita;
													a.sub_total=0;
													for (var i in _.productos){
														p=_.productos[i];
														a.compensatorios=parseFloat(p.dias_atraso)*parseFloat(p.tasa)/365;
														a.sub_total=parseFloat(a.sub_total)+parseFloat(p.deuda)+parseFloat(a.compensatorios);
													}
													a.honorarios=parseFloat(a.sub_total)*(parseFloat(_.porcentaje_honorarios)/100);
													a.iva_sobre_honorarios=parseFloat(a.honorarios)*0.21;
													a.total=parseFloat(a.sub_total)+parseFloat(a.honorarios)+parseFloat(a.iva_sobre_honorarios);
													a.aviso="CAPITAL + COMPENSATORIOS + HONORARIOS + IVA S/HONORARIOS = ANTICIPO 20% + 12 CUOTAS con tasa de refinanciación igual al porcentaje de la tasa de actualización";
											}
										}
									}
									aaa=a;	};
			_.recargar = function () {
													...
													for (var i in productos){
														p=_.productos[i];
														p.deuda=dolar_a_peso(p.deuda, p.dolar, _.dolar);
														p.dias_atraso=calcularDate(_.fecha_actualizacion, p.fecha_mora);
														p.compensatorios=parseFloat(p.dias_atraso)*parseFloat(p.tasa)/365;
														p.punitorios=parseFloat(p.compensatorios)*0.5;
														p.interes=parseFloat(p.punitorios)+parseFloat(p.compensatorios);
														p.iva_interes=parseFloat(p.interes)*0.21;
														p.suma=parseFloat(p.deuda)+parseFloat(p.interes)+parseFloat(p.iva_interes);
														_.sub_total=parseFloat(_.sub_total)+parseFloat(p.suma);
													}
													_.honorarios=parseFloat(_.sub_total)*(parseFloat(_.porcentaje_honorarios)/100);
													_.iva_sobre_honorarios=parseFloat(_.honorarios)*0.21;
			...
			_.reload = function (){
				$http.post('php/productos.php', datos).then(function(res){
													...
													_.a_financiar1 = parseFloat(parseFloat((100 - _.porcentaje.replace('%', ''))/ 100 *_.total).toFixed(2));
												});
			}
			_.iniciar = function () {
			$http.post('php/datos.php', datos)
					   .then(function (res){
										   ...
										   _.cambio_producto();
  										   ...
  										   _.estado=res.estado;
  										   //_.dias_atraso=calcularDiasDate2(_.fecha_actualizacion, _.fecha_mora);
  										   ...
  										   $http.post('php/productos.php', datos).then(function(res){
  											...
  											_.producto="Cuenta-Corriente";
  											_.iniciar();
  											_.producto_quita={aviso:' '};
  										   _.plazo=1;
  										   _.porcentaje='0%'; 
  										   _.quitas=''; 
  										   _.tasa_cuenta_corriente = 35; 
  										   _.tasa_tarjeta = 29;
  										   _.tasa_anual = 36;
  										   _.cambio_producto; 
  										   _.fecha_anticipo = new Date(); 
  										   _.cambio_primer_cuota(); 
  										   _.calcular_cuotas; 
  										   _.quitar();

  										  _.descargar = function (){
  				$http.post('http://'+url+':3001/liquidacion-patagonia',{	
  															banco:_.banco,
  															...
  															quita_descripcion: _.producto_quita.aviso,
  															quita_final: _.producto_quita.total
