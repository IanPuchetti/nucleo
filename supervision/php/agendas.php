...
$consulta = "SELECT DISTINCT deudores.documento, agendas.gestion, agendas.id, deudores.apellido, estados.estado, bancos.dbanco FROM  agendas INNER JOIN deudores ON deudores.documento = agendas.deudor INNER JOIN productos ON agendas.deudor = productos.documento INNER JOIN bancos ON bancos.cbanco = productos.banco  INNER JOIN estados ON productos.estado = estados.id  WHERE agendas.fecha = '$fecha' AND agendas.operador ='$operador' ";
