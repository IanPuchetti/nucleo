var $_GET = {};var url = document.URL.split("/")[2];document.location.search.replace(/\??(?:([^=]+)=([^&]*)&?)/g, function () {
    function decode(s) {
...
.controller('busqueda', function($scope, sharedVars, $http, $timeout) {
		...
		_.gestion = function () {mover_derecha ("derecha", "gestion", "25%", "-50%")};
		_.filtro = function () {mover_derecha ("derecha", "filtro", "25%", "-50%")};
		_.productos = function () {mover_derecha ("derecha", "productos", "25%", "-50%")};
		_.deudor = function () {mover_derecha ("derecha", "deudor", "25%", "-50%")};
		...
		_.traer_productos($_GET['documento']);
...
.controller('gestion', function($scope, sharedVars, socket, $timeout, $http){
		...
		socket.emit('tiempo', {usuario:usuario, apellido: _.data.deudor.apellido, documento: _.data.deudor.documento, telefono: $("#telefonos").val(),  tiempo: _.gestion.horas+':'+_.gestion.minutos+':'+_.gestion.segundos , tipo_gestion : _.gestion.tipo_gestion, color: colorete });
