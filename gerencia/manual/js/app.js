...
function liquidar(documento, cbanco, link){console.log(documento+'-'+cbanco+'-'+link);var banco= $("#liquidacion-banco").children(":selected").attr("id");abrirVentana('liquidacion-'+documento, link+'/?documento='+documento+'&banco='+cbanco);}
...
.controller('busqueda', function($scope, sharedVars, $http) {
		...
		_.gestion = function () {mover_derecha ("derecha", "gestion", "0%", "-100%")};
		_.filtro = function () {mover_derecha ("derecha", "filtro", "0%", "-100%")};
		_.productos = function () {mover_derecha ("derecha", "productos", "0%", "-100%")};
		_.deudor = function () {mover_derecha ("derecha", "deudor", "0%", "-100%")};
		...
		_.traer_deudor($_GET['documento']);_.traer_productos($_GET['documento']);
...
.controller('productos', function($scope, sharedVars, $filter) {
		...
		_.seleccionar= function (numero_operacion){
			...
			mover_derecha ("derecha", "product", "0%", "100%");
